    ...
    public static void importReadings(Component parent) {
        ...
        try (BufferedReader br = new BufferedReader(new FileReader(path))) {
            ...
            while ((line = br.readLine()) != null) {
                lineNum++;
                if (lineNum == 1 && line.toLowerCase().contains("customer")) continue; // skip header
                line = line.trim();
                ...
                for (String[] row : valid) {
                    try {
                        ...
                        PreparedStatement ps = conn.prepareStatement(
                            "INSERT INTO meter_readings (customer_id,reading_date,previous_reading,current_reading,consumption_kwh,units) VALUES (?,?,?,?,?,?)");
                        ps.setInt(1, custId); ps.setString(2, row[1]);
                        ps.setDouble(3, prev); ps.setDouble(4, curr); ps.setDouble(5, consumption); ps.setDouble(6, consumption);
                        ps.executeUpdate();
                        inserted++;
