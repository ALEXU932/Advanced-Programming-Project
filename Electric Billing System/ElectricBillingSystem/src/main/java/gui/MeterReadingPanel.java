...
public class MeterReadingPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        importBtn.addActionListener(e -> { utils.CsvImporter.importReadings(this); loadData(); });
    ...
    private void saveReading(int customerId, String date, double prev, double curr, double consumption) {
        // Some DB schemas include a NOT NULL `units` column (units consumed).
        // Ensure we insert it (set to consumption) to avoid SQL errors when strict SQL mode is enabled.
        ...
        try (Connection conn = DatabaseManager.getInstance().getConnection();
            ...
            ps.setDouble(6, consumption); // `units` = consumption (kWh)
            ps.executeUpdate();
