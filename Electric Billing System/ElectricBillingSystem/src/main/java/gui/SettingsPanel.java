...
public class SettingsPanel extends JPanel {
    ...
    private JPanel buildEmailCard() {
        ...
        utils.EmailService.SmtpConfig cfg = utils.EmailService.loadConfig();
        ...
        saveBtn.addActionListener(e -> {
            saveSetting(utils.EmailService.KEY_HOST,    hostF.getText().trim());
            saveSetting(utils.EmailService.KEY_PORT,    portF.getText().trim());
            saveSetting(utils.EmailService.KEY_USER,    userF.getText().trim());
            saveSetting(utils.EmailService.KEY_PASS,    new String(passF.getPassword()));
            saveSetting(utils.EmailService.KEY_FROM,    fromF.getText().trim());
            saveSetting(utils.EmailService.KEY_ENABLED, String.valueOf(enabledCb.isSelected()));
            saveSetting(utils.EmailService.KEY_TLS,     String.valueOf(tlsCb.isSelected()));
        ...
        testBtn.addActionListener(e -> {
            ...
            new Thread(() -> {
                String result = utils.EmailService.testConnection(to);
    ...
    private void audit(String details) {
        utils.AuditLogger.log(currentUser.getUserId(), currentUser.getUsername(),
            utils.AuditLogger.Action.SETTINGS_CHANGE, details);
    }
}
