...
public class CustomerManagementPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        exportBtn.addActionListener(e -> utils.ExcelExporter.export(this, table, "Customers"));
    }
    ...
    private void deleteSelected() {
        ...
        try (Connection conn = DatabaseManager.getInstance().getConnection()) {
            ...
            // Audit log
            User u = utils.SessionManager.getCurrentUser();
            if (u != null) utils.AuditLogger.log(u.getUserId(), u.getUsername(),
                utils.AuditLogger.Action.DELETE_CUSTOMER,
    ...
    private void showCustomerDialog(Customer existing) {
        ...
        uploadBtn.addActionListener(e -> {
            ...
            String p = utils.ProfilePicUtils.chooseAndSave(dialog, tempId);
            if (p != null) {
                picPath[0] = p;
            ...
            try (Connection conn = DatabaseManager.getInstance().getConnection()) {
                if (isEdit) {
                    ...
                    if (au != null) utils.AuditLogger.log(au.getUserId(), au.getUsername(),
                        utils.AuditLogger.Action.EDIT_CUSTOMER,
                    ...
                    if (picPath[0] != null && uid > 0) {
                        String newPath = utils.ProfilePicUtils.renameTempPic(-1, uid);
                        if (newPath != null) {
                            PreparedStatement pp = conn.prepareStatement(
                                "UPDATE users SET profile_pic=? WHERE user_id=?");
                    ...
                    User au = utils.SessionManager.getCurrentUser();
                    if (au != null) utils.AuditLogger.log(au.getUserId(), au.getUsername(),
                        utils.AuditLogger.Action.ADD_CUSTOMER,
