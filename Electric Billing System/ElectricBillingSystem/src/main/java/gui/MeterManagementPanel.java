...
public class MeterManagementPanel extends JPanel {
    ...
    private void buildUI() {
        ...
        for (int i : new int[]{0,3,6}) table.getColumnModel().getColumn(i).setCellRenderer(center);
        ...
        table.getColumnModel().getColumn(4).setCellRenderer(new DefaultTableCellRenderer() {
            @Override public Component getTableCellRendererComponent(
                ...
                if ("ACTIVE".equals(s))        setForeground(UITheme.SUCCESS);
                else if ("FAULTY".equals(s))   setForeground(UITheme.DANGER);
                else if ("INACTIVE".equals(s)) setForeground(UITheme.WARNING);
        ...
        editBtn.addActionListener(e -> {
            ...
            if (row < 0) { info("Select a meter to edit."); return; }
            showDialog(row);
        });
        ...
        exportBtn.addActionListener(e -> utils.ExcelExporter.export(this, table, "Meters"));
    }
    ...
    private void deleteSelected() {
        int row = table.getSelectedRow();
        if (row < 0) { info("Select a meter to delete."); return; }
        ...
        try (Connection conn = DatabaseManager.getInstance().getConnection();
            ...
            if (au != null) utils.AuditLogger.log(au.getUserId(), au.getUsername(),
                utils.AuditLogger.Action.DELETE_METER, "Deleted meter: " + num + " (ID=" + id + ")");
            loadData();
        } catch (SQLException e) { error("Error: " + e.getMessage()); }
    }
    ...
    private void showDialog(Integer editRow) {
        ...
        JTextField installedF = FormDialog.makeField(isEdit ?
            tableModel.getValueAt(editRow,6).toString() : java.time.LocalDate.now().toString());

        JComboBox<String> typeCb   = FormDialog.makeStringCombo(new String[]{"SINGLE_PHASE","THREE_PHASE","SMART","PREPAID"});
        JComboBox<String> statusCb = FormDialog.makeStringCombo(new String[]{"ACTIVE","INACTIVE","FAULTY","REPLACED"});
        ...
        saveBtn.addActionListener(e -> {
            ...
            if (meterNum.isEmpty()) { dialog.setStatus("Meter Number is required.", true); return; }
            ...
            try (Connection conn = DatabaseManager.getInstance().getConnection()) {
                if (isEdit) {
                    ...
                    User au = utils.SessionManager.getCurrentUser();
                    if (au != null) utils.AuditLogger.log(au.getUserId(), au.getUsername(),
                        utils.AuditLogger.Action.EDIT_METER,
                        "Updated meter ID=" + meterId + ", type=" + typeCb.getSelectedItem() + ", status=" + statusCb.getSelectedItem());
                } else {
                    PreparedStatement ps = conn.prepareStatement(
                    ...
                    User au = utils.SessionManager.getCurrentUser();
                    if (au != null) utils.AuditLogger.log(au.getUserId(), au.getUsername(),
                        utils.AuditLogger.Action.ADD_METER,
