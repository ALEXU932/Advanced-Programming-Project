...
/**
 * AdminProfilePanel provides a settings interface for administrators to manage
 * their profile.
 * Features include profile picture upload/removal, credential updates
 * (username/password),
 * password strength validation, and management of other admin users
 * (add/delete).
 * All changes are logged in the audit system for security tracking.
 */
public class AdminProfilePanel extends JPanel {
    ...
    /**
     * Builds the main UI layout with avatar, profile credentials, and admin
     * management sections.
     */
    ...
    /**
     * Creates the profile picture management card with upload/remove functionality.
     */
    private JPanel buildAvatarCard() {
        ...
        uploadBtn.addActionListener(e -> {
            ...
            if (savePic(path)) {
                ...
                Logic.AuditLogger.log(currentUser.getUserId(), currentUser.getUsername(),
                        Logic.AuditLogger.Action.UPLOAD_PHOTO, "Admin uploaded profile picture");
            } else {
        ...
        removeBtn.addActionListener(e -> {
            ...
            if (savePic(null)) {
                ...
                Logic.AuditLogger.log(currentUser.getUserId(), currentUser.getUsername(),
                        Logic.AuditLogger.Action.UPLOAD_PHOTO, "Admin removed profile picture");
            }
        });
    ...
    /**
     * Creates the credentials management card for updating username and password.
     */
    private JPanel buildProfileCard() {
        ...
        newPassF.getDocument().addDocumentListener(new javax.swing.event.DocumentListener() {
            void update() {
                ...
                if (score < 30) {
                    ...
                    strengthBar.setForeground(UITheme.ACCENT);
        ...
        gbc.gridy = r++;
        form.add(lbl("Current Password *"), gbc);
        gbc.gridy = r++;
        form.add(currPassF, gbc);
        gbc.gridy = r++;
        form.add(lbl("New Password"), gbc);
        gbc.gridy = r++;
        form.add(newPassF, gbc);
        gbc.gridy = r++;
        form.add(strengthBar, gbc);
        gbc.gridy = r++;
        form.add(strengthLbl, gbc);
        gbc.gridy = r++;
        form.add(lbl("Confirm New Password"), gbc);
        gbc.gridy = r++;
        form.add(confirmPassF, gbc);
        gbc.gridy = r++;
        ...
        saveBtn.addActionListener(e -> {
            ...
            if (newUsername.isEmpty()) {
                status(statusLbl, "Username cannot be empty.", false);
                return;
            }
            if (curr.isEmpty()) {
                status(statusLbl, "Current password is required.", false);
                return;
            }
            ...
            try (Connection conn = DatabaseManager.getInstance().getConnection()) {
                ...
                if (changePass) {
                    if (!newP.equals(conf)) {
                        status(statusLbl, "New passwords don't match.", false);
                        return;
                    }
                    String vmsg = Logic.SystemSettings.validatePassword(newP);
                    if (vmsg != null) {
                        status(statusLbl, vmsg, false);
                        return;
                    }
                }
                ...
                if (changePass) {
                    upd.setString(2, PasswordUtils.hash(newP));
                    upd.setInt(3, currentUser.getUserId());
                } else {
                    upd.setInt(2, currentUser.getUserId());
                }
                upd.executeUpdate();
                ...
                currPassF.setText("");
                newPassF.setText("");
                confirmPassF.setText("");
                strengthBar.setValue(0);
                // Audit log
                Logic.AuditLogger.log(currentUser.getUserId(), currentUser.getUsername(),
                        changePass ? Logic.AuditLogger.Action.CHANGE_PASSWORD : Logic.AuditLogger.Action.UPDATE_PROFILE,
                        changePass ? "Admin changed password and/or username"
                                : "Admin updated username to: " + newUsername);
    ...
    /**
     * Creates the admin user management card for viewing, adding, and deleting
     * admin accounts.
     */
    private JPanel buildAdminManagementCard() {
        ...
        deleteAdminBtn.addActionListener(e -> {
            ...
            try (Connection conn = DatabaseManager.getInstance().getConnection();
                ...
                ps.executeUpdate();
                Logic.AuditLogger.log(currentUser.getUserId(), currentUser.getUsername(),
                        Logic.AuditLogger.Action.DELETE_ADMIN,
    ...
    private void showAddAdminDialog(DefaultTableModel model) {
        ...
        saveBtn.addActionListener(e -> {
            ...
            if (username.isEmpty()) {
                dialog.setStatus("Username is required.", true);
                return;
            }
            String vmsg = Logic.SystemSettings.validatePassword(pass);
            if (vmsg != null) {
                dialog.setStatus(vmsg, true);
                return;
            }
            if (!pass.equals(conf)) {
                dialog.setStatus("Passwords don't match.", true);
                return;
            }
            ...
            try (Connection conn = DatabaseManager.getInstance().getConnection()) {
                ...
                dialog.dispose();
                Logic.AuditLogger.log(currentUser.getUserId(), currentUser.getUsername(),
                        Logic.AuditLogger.Action.ADD_ADMIN,
        ...
        for (Component comp : c.getComponents()) {
            if (comp instanceof AvatarPanel && comp != avatarPanel) {
                ...
                if (path != null)
                    ...
                    ap.setImage(null);
                    ap.setInitials(currentUser.getUsername());
                }
                ap.repaint();
            }
            if (comp instanceof Container)
                updateAvatars((Container) comp, path);
        }
    }
