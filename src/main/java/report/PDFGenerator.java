...
// PDF Generator class for creating PDF reports
public class PDFGenerator {
    ...
    private static void addHeader(Document doc, PdfWriter writer, Bill bill, Customer customer, String profilePicPath) throws Exception {
        ...
        Paragraph p2 = new Paragraph(SystemSettings.getCompanyName(), fSub());
        Paragraph p3 = new Paragraph(nvl(customer.getName()), new Font(Font.FontFamily.HELVETICA, 11, Font.BOLD, C_ORANGE));
    ...
    private static void addFooter(Document doc) throws Exception {
        String companyInfo = SystemSettings.getCompanyName()
            + "  |  " + SystemSettings.getCompanyAddress()
            + "  |  " + SystemSettings.getCompanyPhone();
        Paragraph p = new Paragraph(
            companyInfo + "\n" +
            "For inquiries: " + SystemSettings.getCompanyEmail() + "\n" +
    ...
    private static PdfPCell transparentCell() {
        ...
        c.setBackgroundColor(new BaseColor(0, 0, 0, 0));
        return c;
    }
    ...
    private static void addInfoRow(PdfPTable t, String l1, String v1, String l2, String v2) {
    ...
    private static void chargeRow(PdfPTable t, String desc, String qty, String rate, String amount, boolean shaded) {
        BaseColor bg = shaded ? C_LIGHT_GRAY : C_WHITE;
        String[] vals   = {desc, qty, rate, amount};
        int[]    aligns = {Element.ALIGN_LEFT, Element.ALIGN_CENTER, Element.ALIGN_CENTER, Element.ALIGN_RIGHT};
        for (int i = 0; i < 4; i++) {
